...
function Botkit(configuration) {
    ...
    botkit.storage = {
        ...
        goals: {
            get: function(goal_id, cb) {
                cb(null, botkit.memory_store.goals[goal_id]);
            },
            save: function(goal, cb) {
                botkit.log('Warning: using temporary storage. Data will be lost when process restarts.');
                if (goal.id) {
                    botkit.memory_store.goals[goal.id] = goal;
                    cb(null, goal.id);
                } else {
                    cb('No ID specified');
                }
            ...
            all: function(cb) {
                cb(null, botkit.memory_store.goals);
            }
        }
    };
...
//~~~~~~~~~~~~~~~~~~~~~~~ BREAKS HERE
            // configuration.storage.goals &&
            // configuration.storage.goals.get &&
            // configuration.storage.goals.save
        ) {
